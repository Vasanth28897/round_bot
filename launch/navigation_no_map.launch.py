...
def generate_launch_description():
    ...
    nav2_params_file = os.path.join(pkg_share, 'config', 'nav2_no_map_params.yaml')
    rviz_file = os.path.join(pkg_share, 'rviz', 'navigation.rviz')
    map_yaml_file = os.path.join(pkg_share, 'maps', 'sonoma.yaml')  # the map file path can be given in the yaml file, if the map file path mentioned there, no need to give in the nav2_map_server node
    ...
    bring_up = IncludeLaunchDescription(
              PythonLaunchDescriptionSource([os.path.join(
                  get_package_share_directory(package_name),'launch','bringup.launch.py'
              )]),
    ...
    localize_lifecycle_manager = Node(
      ...
      output='screen',
      parameters=[{'autostart': autostart}, {'node_names': lifecycle_nodes_localization}, {'use_sim_time': True}],
    )
    ...
    nav2_lifecycle_nodes_manager = Node(
      ...
      output='screen',
      parameters=[{'autostart': autostart}, {'node_names': lifecycle_nodes_nav2}, {'use_sim_time': True}],
    )
